...
** 其中任务1和任务2共同对shareCount进行修改，可以看到如果没有加调度锁，任务2的对shareCount的修改将丢失。
**********************************************************************************************************/
int shareCount;
...
void task1Entry (void * param) 
{
    ...
    for (;;) 
    {
        ...
        // 为了保证shareCount被正确修改，需要先禁止，再修改
        tTaskSchedDisable();

        // 先读取共享变量值
        var = shareCount;

        // 在这期间可能做了其它事情，如果不禁止调度器，将会导致切换到任务2中
        task1Flag = 1;
        tTaskDelay(1);
        
        // 如果不禁止切换调度器，那么再切换回来时，会发现任务2已经修改了shareCount
        // 此时任务1再修改shareCount，任务2的修改就被覆盖了
        var++;
        shareCount = var;
