...
float  cpuUsage = 0.0;              // CPU占用百分比
tTimer usageCheckTimer;             // 用于cpu检查和任务堆栈占用检查的定时器
void usageCheckTimerFunc (void * arg)
{
    tTaskInfo info;

    tTaskGetInfo(&printerTask, &info);
    tTaskGetInfo(&protocolTask, &info);
    tTaskGetInfo(&tcpipTask, &info);
    tTaskGetInfo(&guiTask, &info);
...
void printerTaskEntry (void * param)
{
    ...
    // 初始化CPU检查定时器
    tTimerInit(&usageCheckTimer, 0, TICKS_PER_SEC, usageCheckTimerFunc, 0, TIMER_CONFIG_TYPE_SOFT);
    tTimerStart(&usageCheckTimer);
    for (;;)
    {
...
void tInitApp (void)
{
    tTaskInit(&printerTask, printerTaskEntry, (void *)0, 0, printerTaskEnv, sizeof(printerTaskEnv));
    tTaskInit(&protocolTask, protocolTaskEntry, (void *)0, 1, protocolTaskEnv, sizeof(protocolTaskEnv));
    tTaskInit(&tcpipTask, tcpipTaskEntry, (void *)0, 2, tcpipTaskEnv, sizeof(tcpipTaskEnv));
    tTaskInit(&guiTask, guiTaskEntry, (void *)0, 3, guiTaskEnv, sizeof(guiTaskEnv));
    tTaskInit(&keySimulatorTask, keySimulatorTaskEntry, (void *)0, 1, keySimulatorTaskEnv, sizeof(keySimulatorTaskEnv));
}
